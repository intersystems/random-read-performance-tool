...
Random Read IO Performance Tool for Caché Database
...
The tool requires an empty pre-expanded CACHE.DAT database to be at least double the size of memory in the server and at least four times the storage controller cache size.
The tool uses the Caché ObjectScript VIEW command which reads database blocks in memory so if you are not getting your expected results then perhaps all the database blocks are already in memory.
